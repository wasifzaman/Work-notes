...
finalize_frame.pack(fill=X) #fill frame to expand buttons inside frame
...
Label(cn_list_frame, text='', width=7).pack(side=LEFT) #for alignment
...
import_cn = Button(cn_list_frame, text='Import', width=10) #import CN from file
export_cn = Button(cn_list_frame, text='Export', width=10) #export CN to file
add_cn.pack() #packed
...
Label(install_file_frame, text='Install file', width=7).pack(side=LEFT) #"Install file" label next to install_file packed LEFT
install_file = Entry(install_file_frame, width=30) #install file entry
...
Label(log_file_frame, text='Log file', width=7).pack(side=LEFT) #"Log file" label next to log_file packed LEFT
log_file = Entry(log_file_frame, width=30) #install file entry
...
run_script = Button(finalize_frame, text='Run script') #button to run script
run_script.pack(fill=X) #packed and filled horizontally to fit the frame
exit = Button(finalize_frame, text='Exit') #button to exit
exit.pack(fill=X) #packed and filled horizontally to fit the frame

exit.config(command=root.destroy)